...
from config import (
    APP_TITLE, WINDOW_WIDTH, WINDOW_HEIGHT, CANVAS_BG, BAR_COLOR,
    COMPARE_COLOR, SWAP_COLOR, SORTED_COLOR, FOUND_COLOR, TEXT_COLOR,
    MUTED_TEXT, PANEL_BG, ROOT_BG, CARD_BG, BORDER_COLOR,
    DEFAULT_ARRAY_SIZE, MIN_ARRAY_SIZE, MAX_ARRAY_SIZE,
    DEFAULT_SPEED, MIN_SPEED, MAX_SPEED, DEFAULT_SEARCH_TARGET,
)
...
from custom_loader import controlled_check, run_in_sandbox, analyze_python_file
from test_broken import test_broken_file
...
class AlgorithmVisualizerApp:
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text="CBORN Algorithm Visualizer", fg="white", bg=PANEL_BG, font=("Segoe UI", 24, "bold")).grid(row=0, column=0, columnspan=10, sticky="w", pady=(0, 10))

        tk.Label(header, text="Algorithm:", fg=TEXT_COLOR, bg=PANEL_BG, font=("Segoe UI", 12, "bold")).grid(row=1, column=0, sticky="w", padx=(0, 8))
        self.algo_combo = ttk.Combobox(header, textvariable=self.algorithm_name, values=list(ALGORITHMS.keys()), state="readonly", width=18)
        ...
        self.start_btn.grid(row=1, column=2, padx=3)
        self.pause_btn = make_button(header, "Pause", self.pause_sorting, "#f59e0b")
        self.pause_btn.grid(row=1, column=3, padx=3)
        self.resume_btn = make_button(header, "Resume", self.resume_sorting, "#0284c7")
        self.resume_btn.grid(row=1, column=4, padx=3)
        self.stop_btn = make_button(header, "Stop", self.stop_sorting, "#dc2626")
        self.stop_btn.grid(row=1, column=5, padx=3)
        self.reset_btn = make_button(header, "Reset", self.reset_data, "#2563eb")
        self.reset_btn.grid(row=1, column=6, padx=3)
        self.random_btn = make_button(header, "Random", self.generate_new_data, "#7c3aed")
        self.random_btn.grid(row=1, column=7, padx=3)

        self.test_btn = make_button(header, "Load Test", self.load_test_data, "#9333ea")
        self.test_btn.grid(row=2, column=0, padx=3, pady=(8, 0))
        self.validate_btn = make_button(header, "Validate", self.validate_current_result, "#059669")
        self.validate_btn.grid(row=2, column=1, padx=3, pady=(8, 0))
        self.load_custom_btn = make_button(header, "Load Custom", self.load_custom_file, "#6d28d9")
        self.load_custom_btn.grid(row=2, column=2, padx=3, pady=(8, 0))
        self.test_broken_btn = make_button(header, "Test Broken", self.on_test_broken, "#ff6b6b")
        self.test_broken_btn.grid(row=2, column=3, padx=3, pady=(8, 0))
        self.controlled_btn = make_button(header, "Controlled", self.run_controlled_check, "#b45309")
        self.controlled_btn.grid(row=2, column=4, padx=3, pady=(8, 0))
        self.sandbox_btn = make_button(header, "Sandbox", self.run_custom_sandbox, "#0f766e")
        self.sandbox_btn.grid(row=2, column=5, padx=3, pady=(8, 0))

        tk.Label(header, text="Size:", fg=TEXT_COLOR, bg=PANEL_BG, font=("Segoe UI", 10, "bold")).grid(row=3, column=0, sticky="w", pady=(8, 0))
        self.size_slider = tk.Scale(header, from_=MIN_ARRAY_SIZE, to=MAX_ARRAY_SIZE, orient="horizontal", variable=self.size_value, command=self.on_size_change, bg=PANEL_BG, fg=TEXT_COLOR, highlightthickness=0, troughcolor="#1f2937", length=120)
        self.size_slider.grid(row=3, column=1, sticky="w", pady=(8, 0))

        tk.Label(header, text="Speed:", fg=TEXT_COLOR, bg=PANEL_BG, font=("Segoe UI", 10, "bold")).grid(row=3, column=2, sticky="w", pady=(8, 0))
        self.speed_slider = tk.Scale(header, from_=MIN_SPEED, to=MAX_SPEED, orient="horizontal", variable=self.speed_value, bg=PANEL_BG, fg=TEXT_COLOR, highlightthickness=0, troughcolor="#1f2937", length=120)
        self.speed_slider.grid(row=3, column=3, sticky="w", pady=(8, 0))

        tk.Label(header, text="Target:", fg=TEXT_COLOR, bg=PANEL_BG, font=("Segoe UI", 10, "bold")).grid(row=3, column=4, sticky="w", pady=(8, 0))
        self.target_entry = tk.Entry(header, textvariable=self.search_target, width=8, font=("Segoe UI", 10))
        self.target_entry.grid(row=3, column=5, sticky="w", pady=(8, 0))

        tk.Label(header, textvariable=self.custom_file_text, fg=MUTED_TEXT, bg=PANEL_BG, font=("Consolas", 8)).grid(row=4, column=0, columnspan=8, sticky="w", pady=(4, 0))
        ...
        self.canvas = tk.Canvas(left_panel, bg=CANVAS_BG, highlightthickness=1, highlightbackground=BORDER_COLOR)
        ...
        tk.Label(variables_card, textvariable=self.variables_text, justify="left", anchor="nw", fg=TEXT_COLOR, bg=CARD_BG, font=("Consolas", 10), wraplength=300).pack(fill="x", padx=10, pady=(0, 10))

        overview_card = make_card(right_panel, "Algorithm Overview", CARD_BG, BORDER_COLOR)
        tk.Label(overview_card, textvariable=self.overview_text, justify="left", anchor="nw", fg=TEXT_COLOR, bg=CARD_BG, font=("Segoe UI", 10), wraplength=300).pack(fill="x", padx=10, pady=(0, 8))
        tk.Label(overview_card, textvariable=self.why_algorithm_text, justify="left", anchor="nw", fg=MUTED_TEXT, bg=CARD_BG, font=("Segoe UI", 10, "italic"), wraplength=300).pack(fill="x", padx=10, pady=(0, 10))

        explanation_card = make_card(right_panel, "Step Explanation", CARD_BG, BORDER_COLOR)
        tk.Label(explanation_card, textvariable=self.step_title_text, justify="left", anchor="nw", fg="white", bg=CARD_BG, font=("Segoe UI", 11, "bold"), wraplength=300).pack(fill="x", padx=10, pady=(0, 8))
        tk.Label(explanation_card, textvariable=self.step_explanation_text, justify="left", anchor="nw", fg=TEXT_COLOR, bg=CARD_BG, font=("Segoe UI", 10), wraplength=300).pack(fill="x", padx=10, pady=(0, 8))
        tk.Label(explanation_card, textvariable=self.step_why_text, justify="left", anchor="nw", fg=MUTED_TEXT, bg=CARD_BG, font=("Segoe UI", 10, "italic"), wraplength=300).pack(fill="x", padx=10, pady=(0, 10))

        code_card = make_card(right_panel, "Code", CARD_BG, BORDER_COLOR)
        self.code_text = tk.Text(code_card, height=18, bg="#0b1220", fg="#cbd5e1", insertbackground="white", relief="flat", font=("Consolas", 10), wrap="none")
        ...
        self.status_label = tk.Label(footer, textvariable=self.status_text, fg=TEXT_COLOR, bg=PANEL_BG, font=("Consolas", 11))
    ...
    def redraw(self, data, active=None, swap=None, sorted_indices=None, found_indices=None):
        draw_data(self.canvas, data, TEXT_COLOR, BAR_COLOR, COMPARE_COLOR, SWAP_COLOR, SORTED_COLOR, FOUND_COLOR, active=active, swap=swap, sorted_indices=sorted_indices, found_indices=found_indices)
    ...
    def apply_idle_state(self, data, message):
        ...
        self.step_explanation_text.set("Henüz algoritma çalıştırılmadı.")
        self.step_why_text.set("Algoritma çalıştığında açıklanacak.")
    ...
    def generate_new_data(self):
        ...
        self.data_manager.regenerate(size, sorted_mode=(algo_type == "searching" and self.algorithm_name.get() == "Binary Search"))

        message = f"Generated new data with {size} elements."
    ...
    def reset_data(self):
        ...
        if self.test_mode_enabled and self.original_test_data:
            ...
            self.apply_idle_state(self.data_manager.get_data(), "Test data reset.")
            return

        self.data_manager.reset()
        self.apply_idle_state(self.data_manager.get_data(), "Data reset.")
    ...
    def load_test_data(self):
        ...
        self.apply_idle_state(test_data, f"Test data loaded for {algorithm_name}")

    def validate_current_result(self):
        if self.current_step_index < 0 or not self.history:
            self.status_text.set("No test state to validate yet.")
            return
        ...
        is_valid, message = validate_algorithm_result(algorithm_name=algorithm_name, final_state=final_state, original_data=base_data, target=target)

        self.status_text.set(("✅ " if is_valid else "❌ ") + message)
    ...
    def render_state(self, step, preserve_status=False):
        ...
        self.redraw(new_data, active=step.get("active", []), swap=step.get("swap", []), sorted_indices=step.get("sorted_indices", []), found_indices=step.get("found_indices", []))
        ...
        self.variables_text.set("\n".join(f"{k} = {v}" for k, v in variables.items()) if variables else "No variables.")
    ...
    def get_delay(self):
        ...
        # ============================================================
        # MAIN.PY icinde load_custom_file metodunu asagidaki ile degistir
        # ============================================================

    def load_custom_file(self):
        """Load custom Python file - bozuk dosyalari da kabul et"""
        ...
        if not ok:
            self.status_text.set("❌ " + message + " (Test Broken ile incele)")
            return

        analysis = analyze_python_file(file_path)

        if analysis["errors"]:
            error_msg = " | ".join(analysis["errors"][:2])
            self.status_text.set("❌ " + error_msg + " (Test Broken ile incele)")
            return

        if analysis["warnings"]:
            warning_msg = " | ".join(analysis["warnings"][:2])
            self.status_text.set("⚠️ " + warning_msg)
            return

        self.status_text.set("✅ File valid. Ready to test!")

    def on_test_broken(self):
        """Test broken file - show errors in new window"""
        if not self.custom_file_path:
            self.status_text.set("❌ Load Custom first!")
            return

        test_broken_file(self.root, self.custom_file_path, ROOT_BG, PANEL_BG)
        self.status_text.set("✅ Analysis complete.")

    def run_controlled_check(self):
        if not self.custom_file_path:
            self.status_text.set("No custom file selected.")
            return

        ok, message = controlled_check(self.custom_file_path)
        self.status_text.set(("✅ " if ok else "❌ ") + message)
    ...
    def run_custom_sandbox(self):
        ...
        if not ok:
            self.status_text.set("❌ " + result.get("message", "Sandbox failed."))
            return
        ...
        if not self.custom_states:
            self.status_text.set("❌ No states returned.")
            return
        ...
        self.why_algorithm_text.set(meta.get("why", "N/A"))
        self.set_code_text("Custom file loaded.")
        ...
        self.status_text.set("✅ Sandbox execution completed.")
